...
----------------------------
--SRPS EXTRA UTILS HERE
--Enhances and requires utils.lua
----------------------------


--example call -- placeRingAroundPoint(BlackHole, 3, 21000, centerStar, 30)
function placeRingAroundPoint(object_type, amount, distance, center_x, center_y, starting_angle) -- if starting angle is null will set randomly
	local angleIncrements = 360 / amount
	
	if type(center_x) == "table" then
		if center_y ~= nil then
			starting_angle = center_y
			center_x, center_y = center_x:getPosition()
		end
	if center_y == nil then
		center_x, center_y = center_x:getPosition()
	end
	...
	if starting_angle == nil then starting_angle = random(0, 360) end
	local current_angle = starting_angle
	
	for n = 1, amount do
		local nextvectorx, nextvectory  = center_x, center_y
		nextvectorx, nextvectory = vectorFromAngle(current_angle, distance)
		object_type():setPosition(center_x + nextvectorx, center_y + nextvectory)
		current_angle = current_angle + angleIncrements
	end
...
end -- function placeRingAroundPoint()

function placeDefensePlatformsAroundPoint(obj_faction, amount, distance, center_x, center_y, starting_angle) -- obj_faction must be a string
	local angleIncrements = 360 / amount
	...
	for n = 1, amount do
		local nextvectorx, nextvectory  = center_x, center_y
		nextvectorx, nextvectory = vectorFromAngle(current_angle, distance)
		CpuShip():setFaction(obj_faction):setTemplate("Defense platform"):orderStandGround():setPosition(center_x + nextvectorx, center_y + nextvectory):setScannedByFaction(obj_faction, true)
		current_angle = current_angle + angleIncrements
	end
...
--given an object and a distance and optionally an angle, return an X and Y based on that
function turnVectorFromObjIntoNewCoords(obj, distance, angle) -- angle optional
if angle == nil then angle = random(0,360) end
	local xvect = nil
	local yvect = nil
	local xact = nil
	local yact = nil
	
	xvect, yvect = obj:getPosition()
	xvect, yvect = vectorFromAngle(angle, distance)
	xact, yact = obj:getPosition()
	outVectX, outVectY = xvect + xact, yvect + yact
	
	

return outVectX, outVectY
end

function turnVectorFromXYIntoNewCoords(x, y, distance, angle) -- angle optional (for x/y coords)
if angle == nil then angle = random(0,360) end
	local xvect = nil
	local yvect = nil
	local xact = nil
...
------
--TIMER, lives in Update
------
--	if var == nil then	var = getScenarioTime() + 60 end
--	if getScenarioTime() > var then
--		dothing
--	var = nil end
